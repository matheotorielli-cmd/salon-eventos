...
function TablaMovimientos({ movimientos, cobros, eventos }) { return <table style={tabla}><thead><tr><th style={th}>Fecha</th><th style={th}>Tipo de movimiento</th><th style={th}>Moneda</th><th style={th}>Monto</th><th style={th}>Referencia</th></tr></thead><tbody>{movimientos.map((movimiento) => { const evento = eventos[cobros[movimiento.referenciaId]?.eventoId]; return <tr key={movimiento.id}><td style={td}>{movimiento.fecha?.toDate ? fecha.format(movimiento.fecha.toDate()) : "—"}</td><td style={td}><span style={tipoBadge}>{movimiento.tipoMovimientoNombre}</span></td><td style={td}>ARS</td><td style={td}>{pesos.format(Number(movimiento.monto || 0))}</td><td style={td}>{evento ? <Link to={`/evento/${evento.id}`} style={link}>{evento.nombreEvento || evento.title || "Evento"}</Link> : "—"}</td></tr> })}{!movimientos.length && <FilaVacia columnas={5} />}</tbody></table> }
