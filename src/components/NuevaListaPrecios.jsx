...
import { guardarListaPrecios, obtenerListaPrecios } from "../services/listasPrecios"

const iso = (fecha) => { const d = new Date(fecha); d.setMinutes(d.getMinutes() - d.getTimezoneOffset()); return d.toISOString().slice(0, 10) }
const itemVacio = () => ({ id: crypto.randomUUID(), nombre: "", presentacion: "", precio: "", activo: true })
...
export default function NuevaListaPrecios() {
  ...
  async function guardar(e) {
    ...
    const normalizar = (items) => items.filter((item) => item.nombre.trim()).map((item) => ({ ...item, nombre: item.nombre.trim(), presentacion: item.presentacion.trim(), precio: Number(item.precio || 0) }))
...
function EditorItems({ titulo, grupo, items, onChange, onAdd, onRemove, presentacion }) {
  return <section className="price-items"><header><div><strong>{titulo}</strong><small>{presentacion ? "Productos disponibles para vender en el evento." : "Servicios o combos incluidos en esta lista."}</small></div><button type="button" onClick={() => onAdd(grupo)}><Plus size={16}/> Agregar</button></header><div className="price-items-list">{items.map((item) => <div className="price-item-row" key={item.id}><label><span>Nombre</span><input value={item.nombre} onChange={(e) => onChange(grupo,item.id,"nombre",e.target.value)} placeholder={presentacion ? "Ej.: Coca-Cola" : "Ej.: Cumpleaños clásico"}/></label>{presentacion && <label><span>Presentación</span><input value={item.presentacion} onChange={(e) => onChange(grupo,item.id,"presentacion",e.target.value)} placeholder="Ej.: 2,25 L"/></label>}<label><span>Precio</span><input type="number" min="1" value={item.precio} onChange={(e) => onChange(grupo,item.id,"precio",e.target.value)} placeholder="$ 0"/></label><button type="button" className="remove" onClick={() => onRemove(grupo,item.id)} title="Quitar"><Trash2 size={17}/></button></div>)}</div></section>
}
